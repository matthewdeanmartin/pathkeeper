...
def test_create_backup_uses_unique_filenames(tmp_path: Path) -> None:
    snapshot = PathSnapshot(
        ["/usr/bin"], ["/home/test/bin"], "/usr/bin", "/home/test/bin"
    )
    first, _ = create_backup(
        snapshot,
        backup_dir=tmp_path,
        os_name="linux",
        tag="manual",
...
def test_create_backup_skips_identical_latest_snapshot(
    ...
    with caplog.at_level(logging.WARNING):
        skipped_path, skipped_records = create_backup(
            snapshot, backup_dir=tmp_path, os_name="linux", tag="manual", note=""
        )
...
def test_prune_backups_honors_manual_and_auto_limits(tmp_path: Path) -> None:
    ...
    _write_backup(
        tmp_path / "2025-03-03T10-00-00_auto.json",
        timestamp="2025-03-03T10:00:00Z",
        tag="auto",
    )
    _write_backup(
        tmp_path / "2025-03-04T10-00-00_auto.json",
        timestamp="2025-03-04T10:00:00Z",
        tag="auto",
    )
    _write_backup(
        tmp_path / "2025-03-05T10-00-00_pre-restore.json",
        timestamp="2025-03-05T10:00:00Z",
        tag="pre-restore",
    )
    config = AppConfig(
        general=GeneralConfig(max_backups=3, max_auto_backups=1, max_manual_backups=1)
    )
    prune_backups(tmp_path, config)
    remaining = [
        record.source_file.name
        for record in list_backups(tmp_path)
