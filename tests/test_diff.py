...
def test_compute_diff_detects_reordered_entry() -> None:
    diff = compute_diff(["/usr/bin", "/usr/local/bin"], ["/usr/local/bin", "/usr/bin"], "linux")
    assert diff.added == []
    assert diff.removed == []
...
def test_compute_diff_windows_trailing_backslash_normalized() -> None:
    ...
    diff = compute_diff(["C:\\Windows\\System32\\"], ["C:\\Windows\\System32"], "windows")
    assert diff == PathDiff(added=[], removed=[], reordered=[])
...
def test_render_diff_all_sections_present() -> None:
    ...
    assert output.index("Added:") < output.index("Removed:") < output.index("Reordered:")
