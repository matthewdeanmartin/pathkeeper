...
def test_analyze_snapshot_detects_duplicates_invalids_and_files(tmp_path: Path) -> None:
    ...
    report = analyze_snapshot(
        system_entries=[str(valid_dir), str(valid_dir), str(file_entry), "$UNEXPANDED_TEST/tool", ""],
        user_entries=[],
        os_name="linux",
        scope=Scope.SYSTEM,
        raw_value=":".join([str(valid_dir), str(valid_dir), str(file_entry), "$UNEXPANDED_TEST/tool", ""]),
    )
