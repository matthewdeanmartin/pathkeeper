...
```bash
uv run pathkeeper gui        # subcommand
uv run pathkeeper --gui      # top-level flag
uv run pathkeeper-gui        # gui-scripts entry point (pyproject.toml)
```
...
```python
with patch("pathkeeper.services.read_current_report",
